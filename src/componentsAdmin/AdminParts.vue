          ...
          <div class="table-responsive">
            <table class="table">
              ...
              <tr v-for="(value, index) in filteredAssets" v-bind:key="index">
                ...
                <td>{{ value.todo }}</td>
                <th><input  value="Eliminar" v-on:click="deleteMyPart(value.id, value.data)" class="delete" type="button"/></th>
              </tr>
              </tbody>
            </table>
          </div>
...
export default {
  ...
  created() {
    this.work_name = window.localStorage.work_name.substring(0, 17); this.work_address = window.localStorage.address.substring(0, 17);
    this.getAllParts();
  },
  methods:{
    ...
    savePart(){
      ...
      manager_api.saveNewPart(this.date, 'Gerente', this.hours, this.material, this.todo, window.localStorage.work_id,  'Gerente')
          .then(res => {
