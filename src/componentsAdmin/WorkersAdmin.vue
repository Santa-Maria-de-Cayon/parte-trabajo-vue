            ...
            <div class="table-responsive">
              <table class="table">
                ...
                <tr v-for="(worker, index) in filteredAllWorkers" v-bind:key="index">
                  ...
                  <td><input type="button" value="Eliminar" class="delete" v-on:click="deleteWorker(worker.id, worker.name)"></td>
                </tr>
                </tbody>
              </table>
            </div>
...
export default {
  ...
  methods:{
    ...
    saveWorker(){                                 //// company_id, company_name, name, email, password
      ...
      try{
        Promise.all([ admin_api.bossSaveWorker ( window.localStorage.id, window.localStorage.company, this.nameWorker, this.emailWorker, this.passwordWorker , 'boss')])
            .then(() => {
