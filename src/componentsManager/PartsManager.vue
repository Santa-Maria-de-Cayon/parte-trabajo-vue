          ...
          <div class="table-responsive">
            <table class="table">
              ...
              <tr v-for="(value, index) in filteredAssets" v-bind:key="index">
                ...
                <td>{{ value.todo }}</td>
                <th><input  value="Eliminar" v-on:click="deleteMyPart(value.id, value.data)" v-if="value.createdfor === this.id_manager" class="delete" type="button"/></th>
              </tr>
              </tbody>
            </table>
          </div>
...
export default {
  ...
  methods: {
    getAllParts(){
      admin_api.adminGetCustomParts( window.localStorage._id_work )
          .then(res => {
            this.allDataParts = res.reverse(); this.parts = res.length;
            this.works_hours = window.localStorage._hours_work;
          });
    },
    savePart(){
       this.showFormCreatePart(); // date, user, time, material,todo, workid, createdfor
       manager_api.saveNewPart(this.date, this.user, this.hours, this.material, this.todo, window.localStorage._id_work,  window.localStorage.id_manager)
           .then(res => {
             console.log(res); window.localStorage._hours_work = res.timer;  this.getAllParts();
             this.$store.dispatch("getGloballWorks");
             this.date = ""; this.hours = ""; this.material = "";  this.todo = "";
           });
    },
    ...
    deleteMyPart(id_part, data_part){
      ...
      if (nameProyect === data_part){
        Promise.all([manager_api.deletePartId(id_part)])
            .then(([response]) => {
              window.localStorage._hours_work = response.hours; // устанавливаем часы для обры в значение с сервера
              this.getAllParts();
            });
  ...
  created() {
    this.work_name = window.localStorage._name_work.substring(0, 17); this.work_address = window.localStorage._adress_work.substring(0, 17);
    this.getAllParts();
    },
  computed: {
    filteredAssets() {
